...
# -*- coding: utf-8 -*-
"""
step5_model_output_validation.py  (GSE89632)  [adaptive dynamic indicators]
================================================================
TRUE model-level validation: feed GSE89632 expression into the existing v10
...
Cross-platform note:
  GSE126848 ratios came from pyDESeq2 (2^log2FC, shrinkage); GSE89632 is
...
Inputs:
  GSE89632_expression_probe.csv, GSE89632_sample_metadata.csv,
  GPL14951_probe_to_gene.csv, node_name_table_hsa04932.xlsx
...
Outputs:
  ...
  GSE89632_model_dynamics.xlsx          (t-half + steady state + early AUC per output)
...
OUT_RATIOS = os.path.join(SCRIPT_DIR, 'GSE89632_node_initial_ratios.xlsx')
OUT_DYN    = os.path.join(SCRIPT_DIR, 'GSE89632_model_dynamics.xlsx')
...
DISEASE_GROUPS = {'NAFL': 'SS', 'NASH': 'NASH'}
REFERENCE_GROUP = 'HC'
USE_USABLE_FLAG = False
# Disease-severity ordering used for progression checks and plotting
STAGE_ORDER = ['Normal', 'NAFL', 'NASH']
...
def main():
    print('=' * 64)
    print('GSE89632 model-level validation (adaptive dynamic indicators)')
    print('=' * 64)
    ...
    print(f'GSE89632 gene means built. Groups: {list(gm.columns)}')
    ...
    for ax, pw in zip(axes.flat, key_outs):
        ...
        ax.grid(alpha=0.3)
    plt.suptitle('GSE89632-driven model: disease-stage P_* dynamics',
