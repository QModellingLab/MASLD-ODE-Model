...
A single script that:
  1. Reads DEG_pydeseq2_NASH_vs_Normal.csv
  2. Runs KEGG + GO BP via the Enrichr API (requires internet)
  3. Produces a 1x2 dot plot (left: GO BP, right: KEGG)
  4. Saves the enrichment results to Excel (all data included)

Input:  DEG_pydeseq2_NASH_vs_Normal.csv (same folder as this script)
...
# recomputing enrichment; not required when plotting from the xlsx).
...
def load_results_from_xlsx(path):
    """Load a previously computed enrichment workbook and skip Enrichr.
    Returns a results dict with the same keys the plotter expects."""
    xl = pd.ExcelFile(path)
    results = {}
    for k in ['KEGG_Up', 'KEGG_Down', 'GO_BP_Up', 'GO_BP_Down']:
        results[k] = pd.read_excel(path, sheet_name=k) if k in xl.sheet_names else pd.DataFrame()
    print(f"  loaded enrichment sheets: {[k for k in results if len(results[k])]}")
    return results


# ============================================================
# Part 3: Dot Plot — font enlarged 2x, dot size proportional to gene count
# ============================================================
...
def make_dotplot(results):
    ...
    # Figure — A4 half-page, extra top/bottom margin reserved for title and legend
    ...
    for ax, (title, up_data, dn_data) in zip(axes, panels):
        ...
        ax.set_yticks(y)
        ax.set_yticklabels(all_terms, fontsize=13)
        ax.set_title(title, fontsize=17, fontweight='bold', loc='left', pad=6)
        ...
        # Gene-count label centered inside each dot; font scales with dot
        # diameter (min 8 -> max 14 pt) so small dots are not overwhelmed;
        # white text with a dark stroke stays legible on dots and background.
        _sq      = np.sqrt(sizes)
        _sq_min  = np.sqrt(MIN_DOT_AREA)
        _sq_max  = np.sqrt(MAX_DOT_AREA)
        # Gene-count label centered inside each dot, font scaled to dot
        # diameter (8 -> 14 pt). Clean npj-style: no outline. White text on
        # sufficiently large dots; dark text on the smallest dots (where the
        # glyph may extend beyond the marker onto the white background).
        _sq      = np.sqrt(sizes)
        _sq_min  = np.sqrt(MIN_DOT_AREA)
        _sq_max  = np.sqrt(MAX_DOT_AREA)
        _small   = MIN_DOT_AREA * 2.2   # area threshold: below -> dark label
        for i in range(n_total):
            if _sq_max > _sq_min:
                _fs = 8.0 + 6.0 * (_sq[i] - _sq_min) / (_sq_max - _sq_min)
            else:
                _fs = 10.0
            _fs = float(np.clip(_fs, 8.0, 14.0))
            _col = 'white' if sizes[i] >= _small else '#222222'
            ax.text(nlog[i], i, str(all_ng_list[i]),
                    fontsize=_fs, va='center', ha='center',
                    color=_col, fontweight='bold', zorder=4)
    ...
    legend_el = [
        Line2D([0],[0], marker='o', color='w', markerfacecolor='#D94F4F',
               markeredgecolor='#333', markersize=10, label='Upregulated'),
        Line2D([0],[0], marker='o', color='w', markerfacecolor='#4F7FD9',
               markeredgecolor='#333', markersize=10, label='Downregulated'),
    ]
    # Size legend: use actual gene_count_to_size → convert to markersize
    for ns in [5, 20, 50]:
        s_area = gene_count_to_size(ns)
        ms = np.sqrt(s_area)  # matplotlib markersize ≈ sqrt(scatter size)
        legend_el.append(
            Line2D([0],[0], marker='o', color='w', markerfacecolor='#aaa',
                   markeredgecolor='#333', markersize=ms,
                   label=f'n = {ns}'))

    fig.legend(handles=legend_el, loc='lower center', ncol=5, fontsize=12,
               frameon=False, bbox_to_anchor=(0.5, -0.03))

    for ext in ['png', 'pdf']:
        out = os.path.join(OUTPUT_DIR, f'Fig3_dotplot_pyDESeq2.{ext}')
...
def main():
    ...
    if os.path.exists(ENRICH_XLSX):
        print(f"\nUsing existing enrichment workbook (Enrichr skipped, no CSV needed):")
        print(f"  {ENRICH_XLSX}")
        results = load_results_from_xlsx(ENRICH_XLSX)
    else:
        print("\nNo enrichment workbook found -> running Enrichr (needs DEG CSV + network).")
