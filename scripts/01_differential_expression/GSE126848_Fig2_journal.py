...
================================
Fig 2 Volcano + 三組 pyDESeq2 比較（Python-only, no batch correction）

Pipeline:
  1. pyDESeq2: NASH vs Normal, NAFL vs Normal, Obese vs Normal
  2. 合併結果 → Excel（含 ratio 欄位，供 ODE 初始值 + Fig3 enrichment 使用）
  3. Volcano plot（NASH vs Normal, S1 標示）
  4. Concordance check vs Suppli Fig.1C

Input:  GSE126848_Gene_counts_mapped.xlsx
        Fig1C_pixel_values.xlsx（可選）
        suppli_S1_genes.csv（可選）
Output: DEG_pydeseq2_3comparisons.xlsx（三組比較 + Merged + Summary）
        DEG_pydeseq2_NASH_vs_Normal.csv（NASH vs Normal，向下相容 Fig3）
...
def make_volcano(res, n_up, n_dn, conc_m, conc_t):
    ...
    fig, ax = plt.subplots(figsize=(7.205, 3.65))   # npj SBA 雙欄寬度 183mm
    ...
    for ext in ['png','pdf']:
        out = os.path.join(OUTPUT_DIR, f'Fig2_volcano_journal.{ext}')
...
def main():
    print("="*60)
    print("  Fig 2: pyDESeq2 三組比較 + Volcano")
    print("="*60)
